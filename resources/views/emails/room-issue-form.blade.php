            ...
            <table width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
                <tr>
                    <td style="background:#0a1931;color:#fff;padding:22px 24px;border-bottom:3px solid #d4af37;">
                        ...
                        <div style="font-size:14px;color:#cbd5e1;margin-top:6px;">Booking <strong>{{ $booking->booking_code }}</strong></div>
                    </td>
                </tr>
                <tr>
                    <td style="padding:24px;">
                        ...
                        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:15px 16px;margin-bottom:18px;line-height:1.7;">
                            <div><strong>Mã booking:</strong> {{ $booking->booking_code }}</div>
                            <div><strong>Phòng đang lưu trú:</strong> {{ $roomNumbers ?: 'Chưa xác định' }}</div>
                            <div><strong>Liên kết có hiệu lực đến:</strong> {{ $expiresAt?->timezone('Asia/Ho_Chi_Minh')->format('d/m/Y H:i') }}</div>
                        </div>

                        <div style="text-align:center;margin:22px 0;">
                            <a href="{{ $formUrl }}" style="display:inline-block;background:#0a1931;color:#fff;text-decoration:none;font-weight:700;padding:12px 18px;border-radius:7px;">Mở biểu mẫu báo sự cố</a>
                        </div>
