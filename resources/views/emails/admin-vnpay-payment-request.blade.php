@php
    ...
    $customerName = $customerName !== '' ? $customerName : 'Quý khách';
        ...
        <div style="background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e5e7eb;">
            ...
            <div style="padding:24px;">
                ...
                <p style="margin:0 0 18px;font-size:15px;line-height:1.6;">
                    {{ $hotelName }} gửi yêu cầu thanh toán cho booking của quý khách. Khi bấm nút thanh toán bên dưới, cổng VNPay sẽ tự hiển thị sẵn số tiền và nội dung thanh toán.
                </p>
                ...
                <div style="border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;margin-bottom:18px;">
                    <table style="width:100%;border-collapse:collapse;font-size:14px;">
                        <tr>
                            ...
                            <td style="padding:11px 14px;text-align:right;font-weight:700;border-bottom:1px solid #eef2f7;">{{ $booking->booking_code }}</td>
                        </tr>
                        <tr>
                            <td style="padding:11px 14px;color:#64748b;border-bottom:1px solid #eef2f7;">Mã giao dịch VNPay</td>
                            <td style="padding:11px 14px;text-align:right;font-weight:700;border-bottom:1px solid #eef2f7;">{{ $payment->txn_ref }}</td>
                        </tr>
                        <tr>
                            <td style="padding:11px 14px;color:#64748b;border-bottom:1px solid #eef2f7;">Nội dung</td>
                            <td style="padding:11px 14px;text-align:right;font-weight:700;border-bottom:1px solid #eef2f7;">{{ $paymentTypeLabel }}</td>
                        </tr>
                        <tr>
                            <td style="padding:11px 14px;color:#64748b;border-bottom:1px solid #eef2f7;">Số tiền cần thanh toán</td>
                            <td style="padding:11px 14px;text-align:right;font-weight:800;color:#dc2626;font-size:18px;border-bottom:1px solid #eef2f7;">{{ $amountText }}</td>
                        </tr>
                        <tr>
                            <td style="padding:11px 14px;color:#64748b;border-bottom:1px solid #eef2f7;">Đơn vị nhận thanh toán</td>
                            <td style="padding:11px 14px;text-align:right;font-weight:700;border-bottom:1px solid #eef2f7;">{{ $hotelName }} qua cổng VNPay</td>
                        </tr>
                        <tr>
                            <td style="padding:11px 14px;color:#64748b;border-bottom:1px solid #eef2f7;">Thời gian lưu trú</td>
                            <td style="padding:11px 14px;text-align:right;font-weight:700;border-bottom:1px solid #eef2f7;">{{ $checkInText }} → {{ $checkOutText }}</td>
                        </tr>
                        <tr>
                            <td style="padding:11px 14px;color:#64748b;">Hiệu lực thanh toán</td>
                            <td style="padding:11px 14px;text-align:right;font-weight:700;">Đến {{ $expiresText }}</td>
                        </tr>
                    </table>
                </div>
                ...
                <div style="text-align:center;margin:24px 0;">
                    <a href="{{ $paymentUrl }}" target="_blank"
                        style="display:inline-block;background:#0f172a;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:12px;font-weight:800;font-size:15px;">
                        Mở thanh toán VNPay
                    </a>
                </div>
                ...
                <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:13px 14px;font-size:13px;line-height:1.6;color:#475569;">
                    ...
                    <span style="word-break:break-all;color:#1d4ed8;">{{ $paymentUrl }}</span>
                </div>

                <p style="margin:18px 0 0;font-size:13px;line-height:1.6;color:#64748b;">
                    Sau khi thanh toán thành công, hệ thống sẽ tự cập nhật trạng thái thanh toán cho booking. Vui lòng không thanh toán lại nếu giao dịch đã thành công.
                </p>
            </div>
        </div>
    </div>
