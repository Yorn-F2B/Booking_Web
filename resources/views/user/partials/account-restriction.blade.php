...
<div class="alert alert-danger border-danger mb-4">
    ...
    @if($accountUser->booking_lock_reason)<div class="small mt-1">Lý do: {{ $accountUser->booking_lock_reason }}</div>@endif
</div>
@endif
@if($isLocked)
<div class="alert alert-warning border-warning mb-4" data-account-countdown="{{ $accountUser->booking_locked_until->toIso8601String() }}">
    <strong>Tài khoản đang bị khóa tạm thời.</strong>
    <div>Khóa đến: {{ $accountUser->booking_locked_until->timezone('Asia/Ho_Chi_Minh')->format('H:i d/m/Y') }}</div>
    <div>Thời gian còn lại: <b data-countdown-output>Đang tính...</b></div>
    @if($accountUser->booking_lock_reason)<div class="small">Lý do: {{ $accountUser->booking_lock_reason }}</div>@endif
</div>
<script>
