        ...
        <div class="card-body p-4">
            ...
            <form id="lateArrivalRequestForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input id="expectedArrivalDate" name="expected_arrival_date" type="date" class="form-control" required
                            data-year-select value="{{ $oldArrivalDate }}" min="{{ \Carbon\Carbon::parse($booking->check_in_date)->format('Y-m-d') }}">
                    </div>
                    <div class="col-md-6">
                        ...
                        <input id="expectedArrivalTime" name="expected_arrival_time" type="text" class="form-control" required
                            data-project-time-picker value="{{ $oldArrivalTime }}" placeholder="HH:mm">
                    </div>
                    <div class="col-12">
                        <label class="form-label">Lý do đến muộn <span class="text-danger">*</span></label>
                        <textarea name="reason" rows="4" class="form-control" required maxlength="2000" placeholder="Ví dụ: chuyến xe bị chậm, máy bay hoãn chuyến...">{{ old('reason', $pendingRequest?->reason) }}</textarea>
                    </div>
                                ...
                                @foreach($pendingRequest->attachments as $attachment)
                                    ...
                                    <div class="col-6 col-md-3 js-existing-attachment" data-attachment-id="{{ $attachment->id }}">
                                        <div class="border rounded p-2 h-100 position-relative bg-white">
                                            ...
                                            @else
                                                <a href="{{ $fileUrl }}" target="_blank" class="d-flex align-items-center justify-content-center text-decoration-none" style="height:110px"><strong>PDF</strong></a>
                                            @endif
                ...
                <div class="d-flex gap-2 mt-4">
                    <button class="btn btn-primary">{{ $pendingRequest ? 'Cập nhật yêu cầu' : 'Gửi yêu cầu' }}</button>
                    <a href="{{ route('bookings.show', $booking) }}" class="btn btn-outline-secondary">Quay lại booking</a>
                </div>
            </form>
        </div>
    </div>
