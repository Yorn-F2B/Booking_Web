...
@section('title', 'Báo sự cố phòng')
@section('heading', 'Báo sự cố phòng')
@section('subheading', 'Chọn phòng gặp sự cố, nhập nội dung và tải ảnh minh chứng cho từng phòng.')
...
@section('content')
    ...
    <div class="booking-summary">
        ...
        <div class="info"><small>Khách đặt</small><strong>{{ $booking->booked_customer_name ?: '---' }}</strong></div>
    </div>
    ...
    @else
        <form method="POST" action="{{ request()->fullUrl() }}" enctype="multipart/form-data" id="guestRoomIssueForm">
            @csrf
            ...
            <div class="room-list">
                @foreach ($booking->bookingRooms as $bookingRoom)
                    ...
                    @endphp
                    <div class="room-choice {{ $blocked ? 'blocked' : '' }}" data-room-card="{{ $roomId }}">
                        <label class="room-selector">
                            ...
                            <span>
                                ...
                                <span style="font-weight:500;color:#64748b;">· {{ $bookingRoom->room->category?->name ?? $booking->roomCategory?->name }}</span>
                                @if($blocked)
                        ...
                        @if(!$blocked)
                            <div class="room-detail" id="roomIssueDetail{{ $roomId }}">
                                <div class="field">
                                    <label>Sự cố của phòng {{ $bookingRoom->room->room_number }}</label>
                                ...
                                <div class="field">
                                    <label>Ảnh minh chứng của phòng {{ $bookingRoom->room->room_number }} <span style="font-weight:500;color:#64748b;">(tối đa 5 ảnh)</span></label>
                                    <input type="file"
                                           id="guestRoomIssueImages{{ $roomId }}"
                                    ...
                                    <div class="camera-actions">
                                        <button type="button"
                                                ...
                                                data-target-input="#guestRoomIssueImages{{ $roomId }}"
