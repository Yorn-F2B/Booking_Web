...
<div class="hotel-modern">
    <div class="hm-top">
        ...
        <div class="hm-actions">
            <a href="{{ $routeBookingCreate }}" class="hm-btn hm-btn-main">
                <i class="bx bx-plus"></i>
                Tạo booking
            </a>

            <a href="{{ $routeBookingIndex }}" class="hm-btn">
                <i class="bx bx-list-ul"></i>
                Danh sách booking
            </a>

            <a href="{{ $routeRoomsIndex }}" class="hm-btn">
                <i class="bx bx-bed"></i>
                Quản lý phòng
            </a>
        </div>
        ...
        <main class="hm-left">
            <section class="hm-card hm-hero">
                <div>
                    <div class="hm-eyebrow">
                        ...
                        {{ $nowSafe->format('d/m/Y H:i') }} · Bảng điều hành khách sạn
                    </div>

                    <h2>{{ $occupancyPercent }}% công suất hôm nay</h2>
                ...
                <div class="hm-room-summary">
                    <div class="hm-card-head p-0 mb-3">
                        <div>
                            ...
                            <p class="hm-card-sub">Tổng {{ $totalRooms }} phòng theo trạng thái thực tế.</p>
                        </div>
                        <span class="hm-icon"><i class="bx bx-door-open"></i></span>
                    </div>

                    <div class="hm-availability-bar">
                        <div class="hm-bar hm-bar-available {{ $percentClass($sellablePercent) }}"></div>
                        <div class="hm-bar hm-bar-occupied {{ $percentClass($occupancyPercent) }}"></div>
                        <div class="hm-bar hm-bar-reserved {{ $percentClass($reservedPercent) }}"></div>
                        <div class="hm-bar hm-bar-not-ready {{ $percentClass($notReadyPercent) }}"></div>
                    </div>
            ...
            <section class="hm-kpi-grid">
                <article class="hm-kpi green">
                    <div>
                        ...
                        <strong>{{ $checkinsToday->count() }}</strong>
                ...
                <article class="hm-kpi blue">
                    <div>
                        ...
                        <strong>{{ $checkoutsToday->count() }}</strong>
                ...
                <article class="hm-kpi lime">
                    <div>
                        ...
                        <strong>{{ $compact($todayRevenue) }}</strong>
                        <small><i class="bx bx-wallet-alt"></i> Tháng này {{ $compact($monthRevenue) }}</small>
                    </div>
            ...
            <section class="hm-two-grid">
                <div class="hm-card">
                    <div class="hm-card-head">
                        ...
                        <span class="hm-badge hm-badge-success">Max {{ $compact($revenueMax) }}</span>
                    </div>

                    <div class="hm-chart">
                        @forelse ($revenueValues as $index => $value)
                            <div class="hm-chart-item">
                                <div class="hm-chart-value">{{ $compact($value) }}</div>
                                <div class="hm-chart-track">
                                    <div class="hm-chart-fill {{ $heightClass($value, $revenueMax) }}"></div>
                                </div>
                                <div class="hm-chart-label">{{ $revenueLabels[$index] ?? '' }}</div>
                            </div>
                        @empty
                            <div class="hm-empty">
                ...
                <div class="hm-card">
                    <div class="hm-card-head">
                        <div>
                            ...
                            <p class="hm-card-sub">Xem hệ thống đang tắc ở trạng thái nào.</p>
                        </div>
                        <span class="hm-icon"><i class="bx bx-git-branch"></i></span>
                    </div>

                    <div class="hm-progress-list">
                        @forelse ($bookingStatusChart as $row)
                            <div>
                                <div class="hm-progress-head">
                                    <span>{{ $row['label'] }}</span>
                                    <span>{{ $row['count'] }} booking · {{ $row['percent'] }}%</span>
                                </div>
                                <div class="hm-progress-track">
                                    <span class="hm-progress-fill {{ $percentClass($row['percent']) }}"></span>
                                </div>
                            </div>
                        @empty
                            <div class="hm-empty">
            ...
            <section class="hm-card">
                ...
                <div class="hm-timeline">
                    @forelse ($todayTimeline as $item)
                        <div class="hm-timeline-item hm-tone-{{ $item['tone'] }}">
                            <div class="hm-time">{{ $item['time'] }}</div>
                            ...
                            <div>
                                <div class="hm-main-text">{{ $item['title'] }}</div>
                                <div class="hm-sub-text">{{ $item['text'] }}</div>
                            </div>

                            @if (!empty($item['url']))
                                <a href="{{ $item['url'] }}" class="hm-task-action">Xem</a>
                            @endif
                        </div>
                    @empty
                        <div class="hm-empty">
            ...
            <section class="hm-card">
                ...
                <div class="hm-room-board">
                    @forelse ($floorMap as $floorNumber => $rooms)
                        <div class="hm-floor">
                            <div class="hm-floor-label">Tầng {{ $floorNumber ?: 'N/A' }}</div>

                            <div class="hm-room-grid">
                                @foreach ($rooms as $room)
                                    <a href="{{ Route::has('admin.rooms.index') ? route('admin.rooms.index', ['floor' => $room->floor_number]) : '#' }}"
                                        class="hm-room {{ $roomClass[$room->status] ?? '' }}"
                                        title="Phòng {{ $room->room_number }} - {{ $roomStatusLabels[$room->status] ?? $room->status }}">
                                        <span class="hm-room-top">
                                            {{ $room->room_number }}
                                            <span class="hm-room-dot"></span>
                                        </span>
                                        <span class="hm-room-meta">{{ $roomStatusLabels[$room->status] ?? $room->status }}</span>
                                        <span class="hm-room-meta">{{ $room->category->name ?? 'Chưa có hạng' }}</span>
                                    </a>
                                @endforeach
                            </div>
                        </div>
        ...
        <aside class="hm-right">
            <section class="hm-card">
                ...
                <div class="hm-task-list">
                    @forelse ($urgentAlerts->take(4) as $alert)
                        <div class="hm-task {{ ($alert['level'] ?? 'warning') === 'danger' ? 'danger' : 'warning' }}">
                            <div class="hm-task-icon">
                                <i class="{{ $alert['icon'] ?? 'bx bx-bell' }}"></i>
                            </div>

                            <div>
                                <div class="hm-task-title">{{ $alert['title'] ?? 'Cần xử lý' }}</div>
                                <div class="hm-task-text">{{ $alert['message'] ?? '' }}</div>
                            </div>

                            @if (!empty($alert['url']))
                                <a href="{{ $alert['url'] }}" class="hm-task-action">{{ $alert['action'] ?? 'Xem' }}</a>
                            @else
                                <span class="hm-task-action">{{ $alert['action'] ?? 'Xem' }}</span>
                            @endif
                        </div>
                    @empty
                        <div class="hm-empty">
            ...
            <section class="hm-card">
                ...
                <div class="hm-task-list">
                    @forelse ($systemWarnings->take(3) as $warning)
                        <div class="hm-task warning">
                            ...
                            <div>
                                <div class="hm-task-title">{{ $warning['title'] ?? 'Bất thường' }}</div>
                                <div class="hm-task-text">{{ $warning['message'] ?? '' }}</div>
                            </div>

                            @if (!empty($warning['url']))
                                <a href="{{ $warning['url'] }}" class="hm-task-action">{{ $warning['action'] ?? 'Xem' }}</a>
                            @else
                                <span class="hm-task-action">{{ $warning['action'] ?? 'Xem' }}</span>
                            @endif
                        </div>
                    @empty
                        <div class="hm-empty">
            ...
            <section class="hm-card">
                ...
                <div class="hm-finance-grid">
                    ...
                    <div class="hm-finance">
                        <span>Dịch vụ</span>
                        <strong>{{ $money($serviceRevenueToday) }}</strong>
                    </div>

                    <div class="hm-finance">
                        <span>Cần thu</span>
                        <strong>{{ $money($receivableAmount) }}</strong>
                    </div>

                    <div class="hm-finance">
                        <span>Chưa TT</span>
                        <strong>{{ $unpaidActiveBookings }}</strong>
                    </div>

                    <div class="hm-finance">
                        <span>Đã cọc</span>
                        <strong>{{ $partialActiveBookings }}</strong>
                    </div>
                </div>
            </section>
            ...
            <section class="hm-card">
                ...
                <div class="hm-finance-grid">
                    ...
                    <div class="hm-finance">
                        <span>Chờ duyệt</span>
                        <strong>{{ $inspectionReported }}</strong>
                    </div>

                    <div class="hm-finance">
                        <span>Item chờ duyệt</span>
                        <strong>{{ $inspectionPendingItems }}</strong>
                    </div>

                    <div class="hm-finance">
                        <span>Phân công</span>
                        <strong>{{ $roomCompleted }}/{{ $roomAssigned }}</strong>
                    </div>
                </div>
                ...
                <div class="hm-progress-list">
                    <div>
                        <div class="hm-progress-head">
                            ...
                            <span>{{ $assignmentPercent }}%</span>
                        </div>
                        <div class="hm-progress-track">
                            <span class="hm-progress-fill {{ $percentClass($assignmentPercent) }}"></span>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="hm-card">
                ...
                <div class="hm-progress-list">
                    @forelse ($categoryRows->take(5) as $row)
                        @php
                        ...
                        <div>
                            ...
                            <div class="hm-progress-track">
                                <span class="hm-progress-fill {{ $percentClass($percent) }}"></span>
                            </div>
                        </div>
                    @empty
                        <div class="hm-empty">
