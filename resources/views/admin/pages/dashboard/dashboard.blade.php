...
<style>
    ...
    .exec-source-row,.exec-money-row{display:grid;grid-template-columns:minmax(120px,1fr) 2fr auto;gap:10px;align-items:center;margin-bottom:12px;font-size:12px;color:inherit;text-decoration:none;border-radius:8px;padding:4px}.exec-source-row:hover,.exec-money-row:hover{background:#f4f7fa}.exec-source-row .exec-track,.exec-money-row .exec-track{height:8px}.exec-source-row strong,.exec-money-row strong{font-size:12px;white-space:nowrap}
...
<div class="exec-dashboard">
    ...
    <section class="exec-range">
        <form method="GET" action="{{ route('admin.dashboard') }}" class="exec-range-form">
            <div class="field"><label>Từ ngày</label><input type="date" name="from" id="dashboardRangeFrom" class="form-control" value="{{ $from->format('Y-m-d') }}"></div>
            <div class="field"><label>Đến ngày</label><input type="date" name="to" id="dashboardRangeTo" class="form-control" value="{{ $to->format('Y-m-d') }}"></div>
    ...
    <section class="exec-kpis">
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'revenue','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}">
            <div class="exec-kpi-icon"><i class="bx bx-wallet"></i></div><div class="label">Tiền đã thu trong kỳ</div><div class="value">{{ $compactMoney($revenue) }}</div>
            ...
            <div class="sub">Giao dịch thanh toán thành công · kỳ trước: {{ $compactMoney($previousRevenue) }} · bấm để xem từng giao dịch</div>
        </a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'booking_value','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-receipt"></i></div><div class="label">Giá trị booking phát sinh</div><div class="value">{{ $compactMoney($bookingValue) }}</div><div class="sub">Không tính booking đã hủy · dựa trên đơn tạo trong kỳ · bấm để xem chi tiết</div></a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'occupancy','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-bed"></i></div><div class="label">Công suất phòng bình quân</div><div class="value">{{ number_format((float)data_get($occupancy,'rate',0),1,',','.') }}%</div><div class="sub">{{ data_get($occupancy,'occupied_room_days',0) }} / {{ data_get($occupancy,'capacity_room_days',0) }} phòng-ngày · bấm để xem từng phòng</div></a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'new_bookings','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-calendar-check"></i></div><div class="label">Booking mới</div><div class="value">{{ $newBookings }}</div><div class="sub">Hoàn tất/trả phòng: {{ $completedBookings }} · Hủy: {{ $cancelledBookings }} · No-show: {{ $noShowCount }} · bấm để xem</div></a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'receivables','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-credit-card"></i></div><div class="label">Công nợ liên quan kỳ</div><div class="value">{{ $compactMoney($receivableAmount) }}</div><div class="sub">{{ $receivableBookings }} booking còn số tiền phải thu · bấm để xem</div></a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'active_stays','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-user-check"></i></div><div class="label">Khách đang lưu trú</div><div class="value">{{ $activeGuests }}</div><div class="sub">{{ $activeStays }} booking đang checked-in · bấm để xem danh sách</div></a>
    </section>

    <section class="exec-grid">
        <article class="exec-card">
            <div class="exec-card-head"><div><h2>Xu hướng tiền thu vào</h2><p>Giao dịch thanh toán thành công theo đúng thời điểm thu.</p></div><a class="exec-pill text-decoration-none" href="{{ route('admin.dashboard.detail',['metric'=>'revenue','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}">Xem giao dịch</a></div>
        ...
        <article class="exec-card">
            ...
            <div class="exec-room-now"><a class="exec-room-stat text-decoration-none text-dark" href="{{ route('admin.dashboard.detail',['metric'=>'room_status','group'=>'available','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><strong>{{ $currentAvailableRooms }}</strong><span>Trống</span></a><a class="exec-room-stat text-decoration-none text-dark" href="{{ route('admin.dashboard.detail',['metric'=>'room_status','group'=>'occupied','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><strong>{{ $currentOccupiedRooms }}</strong><span>Đang ở</span></a><a class="exec-room-stat text-decoration-none text-dark" href="{{ route('admin.dashboard.detail',['metric'=>'room_status','group'=>'reserved','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><strong>{{ (int)($roomStatuses['reserved'] ?? 0) }}</strong><span>Đã giữ</span></a><a class="exec-room-stat text-decoration-none text-dark" href="{{ route('admin.dashboard.detail',['metric'=>'room_status','group'=>'not_ready','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><strong>{{ $currentNotReadyRooms }}</strong><span>Chưa sẵn sàng</span></a></div>
            @forelse($categoryRows as $row)<a class="exec-progress-row d-block text-decoration-none text-dark" href="{{ route('admin.dashboard.detail',['metric'=>'category_occupancy','group'=>$row['id'],'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-progress-meta"><strong>{{ $row['name'] }}</strong><span>{{ $row['rate'] }}% · {{ $row['room_count'] }} phòng</span></div><div class="exec-track"><div class="exec-fill" style="width:{{ max(0,min(100,$row['rate'])) }}%"></div></div></a>@empty<div class="exec-empty">Chưa có dữ liệu hạng phòng.</div>@endforelse
        </article>
    </section>
    ...
    <section class="exec-grid equal">
        <article class="exec-card">
            ...
            @forelse($sourceRows as $row)<a class="exec-source-row" href="{{ route('admin.dashboard.detail',['metric'=>'booking_source','group'=>$row['key'],'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><span>{{ $row['label'] }}</span><div class="exec-track"><div class="exec-fill lime" style="width:{{ (($row['count']/$sourceMax)*100) }}%"></div></div><strong>{{ $row['count'] }}</strong></a>@empty<div class="exec-empty">Chưa có booking trong kỳ.</div>@endforelse
        </article>
        <article class="exec-card">
            ...
            @forelse($surchargeCollection as $row)<a class="exec-money-row" href="{{ route('admin.dashboard.detail',['metric'=>'surcharges','group'=>$row['key'],'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><span>{{ $row['label'] }}</span><div class="exec-track"><div class="exec-fill orange" style="width:{{ (($row['total']/$surchargeMax)*100) }}%"></div></div><strong>{{ $compactMoney($row['total']) }}</strong></a>@empty<div class="exec-empty">Không phát sinh phụ thu trong kỳ.</div>@endforelse
        </article>
    </section>

    <section class="exec-grid equal">
        <article class="exec-card">
            <div class="exec-card-head"><div><h2>Cơ cấu tiền thu vào</h2><p>Phân theo phương thức/provider của giao dịch thành công.</p></div><span class="exec-pill">{{ $compactMoney($grossRevenue) }}</span></div>
            @forelse($providerCollection as $row)<a class="exec-money-row" href="{{ route('admin.dashboard.detail',['metric'=>'payment_provider','group'=>$row['key'],'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><span>{{ $row['label'] }}</span><div class="exec-track"><div class="exec-fill blue" style="width:{{ (($row['total']/$providerMax)*100) }}%"></div></div><strong>{{ $compactMoney($row['total']) }}</strong></a>@empty<div class="exec-empty">Chưa có khoản thu trong kỳ.</div>@endforelse
        </article>
        <article class="exec-card">
            ...
            <div class="exec-status-grid">
                @foreach($statusLabels as $status=>$label)
                    ...
                    <a class="exec-status text-decoration-none text-dark" href="{{ route('admin.dashboard.detail',['metric'=>'booking_status','group'=>$status,'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="n">{{ $count }}</div><div class="t">{{ $label }} · {{ number_format(($count/$statusTotal)*100,1,',','.') }}%</div></a>
                @endforeach
            </div>
        </article>
    </section>
...
document.addEventListener('DOMContentLoaded', function () {
    const from = document.getElementById('dashboardRangeFrom');
    const to = document.getElementById('dashboardRangeTo');
    const sync = () => {
        if (!from || !to || !from.value) return;
        to.min = from.value;
        if (to._flatpickr) to._flatpickr.set('minDate', from.value);
        if (!to.value || to.value < from.value) {
            to.value = from.value;
            if (to._flatpickr) to._flatpickr.setDate(from.value, false);
        }
    };
    from?.addEventListener('change', sync);
    from?.addEventListener('project-date-change', sync);
    sync();
});
</script>
