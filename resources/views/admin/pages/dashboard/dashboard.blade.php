...
<div class="exec-dashboard">
    ...
    <section class="exec-kpis">
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'revenue','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}">
            <div class="exec-kpi-icon"><i class="bx bx-wallet"></i></div><div class="label">Doanh thu thuần thực thu</div><div class="value">{{ $compactMoney($revenue) }}</div>
            ...
            <div class="sub">Thu vào {{ $compactMoney($grossRevenue) }} − đã hoàn {{ $compactMoney($refundAmount) }} · kỳ trước: {{ $compactMoney($previousRevenue) }}</div>
        </a>
        ...
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'discounts','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-purchase-tag-alt"></i></div><div class="label">Ưu đãi / giảm giá</div><div class="value">{{ $compactMoney($discountAmount) }}</div><div class="sub">Tổng discount snapshot của booking tạo trong kỳ · bấm để xem</div></a>
    </section>

    <section class="exec-kpis">
        <article class="exec-kpi"><div class="exec-kpi-icon"><i class="bx bx-user-check"></i></div><div class="label">Khách đang lưu trú</div><div class="value">{{ $activeGuests }}</div><div class="sub">{{ $activeStays }} booking đang checked-in</div></article>
        <article class="exec-kpi"><div class="exec-kpi-icon"><i class="bx bx-line-chart"></i></div><div class="label">Giá trị booking trung bình</div><div class="value">{{ $compactMoney($averageBookingValue) }}</div><div class="sub">Giá trị booking chưa hủy / {{ $nonCancelledBookings }} booking hợp lệ trong kỳ</div></article>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'pending_payments','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-time-five"></i></div><div class="label">Thanh toán chờ xử lý</div><div class="value">{{ $pendingPayments }}</div><div class="sub">Payment pending tạo trong kỳ · bấm để xem</div></a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'failed_payments','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-error-circle"></i></div><div class="label">Thanh toán thất bại</div><div class="value">{{ $failedPayments }}</div><div class="sub">Payment failed trong kỳ · bấm để xem</div></a>
        <article class="exec-kpi"><div class="exec-kpi-icon"><i class="bx bx-wrench"></i></div><div class="label">Sự cố phòng đang mở</div><div class="value">{{ $openRoomIssues }}</div><div class="sub">Cần theo dõi/giải quyết trong vận hành hiện tại</div></article>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'failed_emails','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-envelope"></i></div><div class="label">Email lỗi trong kỳ</div><div class="value">{{ $failedEmails }}</div><div class="sub">Gửi thất bại được hệ thống ghi log · bấm để xem</div></a>
        <a class="exec-kpi" href="{{ route('admin.dashboard.detail',['metric'=>'refunds','from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')]) }}"><div class="exec-kpi-icon"><i class="bx bx-undo"></i></div><div class="label">Tiền đã hoàn khách</div><div class="value">{{ $compactMoney($refundAmount) }}</div><div class="sub">Các khoản hoàn đã xác nhận hoàn tất trong kỳ · bấm để xem</div></a>
    </section>

    <section class="exec-grid">
        <article class="exec-card">
            <div class="exec-card-head"><div><h2>Xu hướng tiền thu vào</h2><p>Giao dịch payment thành công theo thời điểm thu; khoản hoàn được theo dõi riêng ở KPI phía trên.</p></div><span class="exec-pill">{{ $periodLabel }}</span></div>
    ...
    <section class="exec-grid equal">
        <article class="exec-card">
            <div class="exec-card-head"><div><h2>Cơ cấu tiền thu vào</h2><p>Phân theo phương thức/provider của giao dịch thành công, trước khi trừ hoàn tiền.</p></div><span class="exec-pill">{{ $compactMoney($grossRevenue) }}</span></div>
