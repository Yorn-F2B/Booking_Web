                ...
                <div class="main-stack">
                    <section class="card-clean">
                        ...
                        <div class="operation-list">
                            ...
                            @elseif (in_array($booking->status, ['checked_out', 'completed']))
                                <div class="operation-row">
                                    <div class="operation-row-head">
                                        <div>
                                            <div class="operation-row-title">Xuất hóa đơn</div>
                                            <div class="text-muted small">In hóa đơn thanh toán cho khách hàng.</div>
                                        </div>
                                    </div>
                                    ...
                                    @if ($existingInvoice)
                                        <div class="alert alert-success small mb-3">
                                            <div class="fw-bold mb-1">Hóa đơn đã được tạo</div>
                                            Mã hóa đơn: <strong>{{ $existingInvoice->invoice_code }}</strong>
                                            <br>
                                            Ngày xuất: {{ $existingInvoice->issued_at ? \Carbon\Carbon::parse($existingInvoice->issued_at)->format('d/m/Y H:i') : '---' }}
                                        </div>
                                        
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('admin.invoices.show', $existingInvoice) }}" class="btn btn-info flex-grow-1">
                                                <i class="bx bx-eye me-1"></i> Xem hóa đơn
                                            </a>
                                            <a href="{{ route('admin.invoices.print', $existingInvoice) }}" class="btn btn-primary flex-grow-1" target="_blank">
                                                <i class="bx bx-printer me-1"></i> In hóa đơn
                                            </a>
                                        </div>
                                    @else
                                        <form action="{{ route('admin.invoices.create-from-booking', $booking->id) }}" method="GET">
                                            <button type="submit" class="btn btn-success w-100">
                                                <i class="bx bx-file me-1"></i> Tạo hóa đơn mới
                                            </button>
                                        </form>
                                    @endif
                                </div>
