                            ...
                            @if ($booking->status == 'confirmed')
                                <div class="operation-row">
                                    ...
                                    <form action="{{ route('admin.bookings.check-in', $booking->id) }}" method="POST" enctype="multipart/form-data"
                                        ...
                                        @php
                                            $depositEstimatedTotal = (float) $booking->estimated_total;
                                            $depositAmount = (float) $booking->deposit_amount;
                                            $requiredDeposit = $depositEstimatedTotal * 0.3;
                                            $isDepositSufficient = $depositAmount >= $requiredDeposit;
                                        @endphp

                                        @if (!$isDepositSufficient)
                                            <div class="alert alert-danger small mb-3">
                                                <strong>Chưa đủ cọc 30%!</strong> Booking chưa thanh toán đủ số tiền cọc bắt buộc (thiếu {{ number_format($requiredDeposit - $depositAmount, 0, ',', '.') }}đ).<br>
                                                Vui lòng yêu cầu khách thanh toán thêm tại mục "Thanh toán". Nếu muốn ngoại lệ cho nhận phòng, Quản lý phải nhập lý do bên dưới.
                                                <textarea name="override_deposit_reason" class="form-control mt-2" rows="2" placeholder="Nhập lý do ngoại lệ để tiếp tục..."></textarea>
                                            </div>
                                        @endif
    ...
    <div class="compact-panel-body">
                    ...
                    <tbody>
                        @foreach ($booking->guests as $guest)
                            <tr>
                                ...
                                <td>
                                    @if($guest->cccd_image_path)
                                        <a href="{{ Storage::url($guest->cccd_image_path) }}" target="_blank" class="badge bg-info text-decoration-none">Xem ảnh</a>
                                    @else
        ...
        >
            ...
            <div class="row g-2">
                ...
                <div class="col-md-2">
                    <label class="form-label small">
                        CCCD/Hộ chiếu <span class="text-danger">*</span>
                    </label>
                ...
                <div class="col-md-3">
                    <label class="form-label small">
                        Phòng lưu trú <span class="text-danger">*</span>
                    </label>

                    <select name="booking_room_id" class="form-select form-select-sm" required>
