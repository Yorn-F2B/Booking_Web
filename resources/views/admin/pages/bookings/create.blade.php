...
@if ($errors->any())
            ...
            <form action="{{ route('admin.bookings.store') }}" method="POST" id="bookingCreateForm">

                @csrf
                <input type="hidden" name="confirm_adjacent_fallback" id="confirmAdjacentFallback" value="{{ old('confirm_adjacent_fallback', 0) }}">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="customer_cccd" id="customerCccd"
                                        ...
                                        value="{{ old('customer_cccd') }}" placeholder="Nhập đúng 12 số CCCD" required>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="room_category_id" id="roomCategorySelect"
                                        class="form-select category-stock-select @error('room_category_id') is-invalid @enderror" required>
                                                ...
                                                <div class="mb-3" data-promotion-group data-promotion-type="{{ $promotionType }}" data-promotion-limit="{{ $typeConfig['limit'] ?? '' }}">
                                                    <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                                                        <div>
                                                            <div class="fw-bold">
                                                                {{ $typeConfig['label'] }}
                                                                <span class="badge {{ $typeConfig['badge'] }} ms-1" data-promotion-group-count>
                                                                    ...
                                                                    <div class="ms-1">
                                                                        ...
                                                                        <div class="promotion-meta mt-1">
                                                                            ...
                                                                            @if ($promotion->requires_note || $promotion->promotion_type == 'support_discount')
                                                                                · Cần nhập lý do
                                                                            @endif
                                                                            · {{ $promotion->is_stackable ? 'Có thể dùng chung' : 'Chỉ dùng một mình' }}
                                                                        </div>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content border-0 shadow-lg">
                            ...
                            <div class="modal-body pt-3">
                                <p class="mb-3">
                                    Hạng <strong>{{ $adjacentRoomWarning['category_name'] ?? 'đã chọn' }}</strong>
                                    còn đủ <strong>{{ $adjacentRoomWarning['requested_quantity'] ?? 0 }} phòng</strong>,
                                    nhưng không có đủ {{ $adjacentRoomWarning['requested_quantity'] ?? 0 }} phòng nằm liền nhau.
                                </p>

                                <div class="alert alert-info mb-3">
                                    Có thể ghép tối đa
                                    <strong>{{ $adjacentRoomWarning['max_adjacent_count'] ?? 1 }} phòng cạnh nhau</strong>
                                    và bố trí các phòng còn lại gần nhất có thể.
                                </div>

                                <div class="border rounded-3 overflow-hidden">
                                    @foreach (($adjacentRoomWarning['lines'] ?? []) as $line)
                                        <div class="d-flex justify-content-between gap-3 px-3 py-2 {{ !$loop->last ? 'border-bottom' : '' }}">
                                            <div>
                                                <strong>{{ $line['label'] ?? 'Phòng' }}</strong>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const eligiblePromotionsUrl = @json(route('admin.bookings.eligible-promotions'));
            const customerAccountLookupUrl = @json(route('admin.bookings.check-customer-account'));
