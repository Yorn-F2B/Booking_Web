<header class="sticky top-0 z-30 flex items-center justify-between px-6 py-4 bg-white border-b shadow-sm">
    ...
    <div class="flex items-center space-x-4">
        ...
        <a href="{{ url('/') }}" target="_blank" class="text-sm text-gray-600 hover:text-blue-600 flex items-center space-x-1">
            <i class="bx bx-home"></i>
            <span class="hidden md:inline">Về website</span>
        </a>
        ...
        <div x-data="{ dropdownOpen: false }" class="relative">
            <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-2 text-sm focus:outline-none">
                ...
                <span class="hidden md:block font-medium text-gray-700">{{ Auth::user() ? Auth::user()->name : 'Admin User' }}</span>
            ...
            <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="absolute right-0 z-20 w-48 py-2 mt-2 bg-white rounded-md shadow-xl"
                ...
                <a href="{{ url('/') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" target="_blank">Xem trang Web</a>
