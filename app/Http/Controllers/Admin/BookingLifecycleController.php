...
class BookingLifecycleController extends Controller
{
    ...
    public function checkIn(Request $request, Booking $booking)
    {
        ...
        $data = $request->validate([
            ...
            'override_deposit_reason' => 'nullable|string|max:1000',
        ]);
        ...
        if ($request->hasFile('cccd_image')) {
            $path = $request->file('cccd_image')->store('cccds', 'public');
            $booking->customer->update(['cccd_image_path' => $path]);
        }

        foreach ($booking->bookingRooms as $bookingRoom) {
            if ($bookingRoom->room && $bookingRoom->room->status === 'cleaning') {
                return back()->withInput()->with('error', 'Phòng ' . $bookingRoom->room->room_number . ' đang dọn dẹp, không thể check-in lúc này.');
            }
        }

        $estimatedTotal = (float) $booking->estimated_total;
        $depositAmount = (float) $booking->deposit_amount;
        $requiredDeposit = $estimatedTotal * 0.3;

        if ($depositAmount < $requiredDeposit) {
            if (empty($data['override_deposit_reason'])) {
                return back()->withInput()->with('error', 'Booking chưa đóng đủ 30% cọc (thiếu ' . number_format($requiredDeposit - $depositAmount, 0, ',', '.') . 'đ). Yêu cầu đóng thêm hoặc nhập lý do ngoại lệ để tiếp tục.');
            }
        }
        ...
        try {
            ...
            if ($depositAmount < $requiredDeposit && !empty($data['override_deposit_reason'])) {
                $actionNote .= ' Ngoại lệ check-in chưa đủ cọc. Lý do: ' . $data['override_deposit_reason'] . '.';
