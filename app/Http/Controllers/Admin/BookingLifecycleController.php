...
class BookingLifecycleController extends Controller
{
    ...
    private function handleChangeRoomCategory(
        ...
        foreach ($oldBookingRooms as $index => $bookingRoom) {
            ...
            app(BookingRoomPeriodService::class)->changeFrom(
                $booking, $bookingRoom, $newRoom->id, $newPrice,
                $booking->status === 'checked_in' ? now('Asia/Ho_Chi_Minh')->toDateString() : $booking->check_in_at->toDateString()
            );
    ...
    private function repriceCurrentBooking(Booking $booking): array
    {
        ...
        // Repricing remains responsible for promotion eligibility, while the final monetary
        // amount is always obtained from BookingCalculatorService via BookingFinancialService.
        $oneNightRoomTotal = (float) $booking->bookingRooms->sum(fn (BookingRoom $room) => (float) $room->price_at_booking);
