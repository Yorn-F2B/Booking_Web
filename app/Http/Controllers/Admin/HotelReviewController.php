...
class HotelReviewController extends Controller
{
    ...
    public function approve(HotelReview $hotelReview)
    {
        DB::transaction(function () use ($hotelReview) {
            ...
            BookingLog::create([
                ...
                'description' => 'Admin duyệt đánh giá khách sạn ' . $hotelReview->rating . '/5 sao.',
            ]);
        });

        return back()->with('success', 'Đã duyệt đánh giá. Đánh giá sẽ hiển thị ở trang khách.');
    }

    public function hide(Request $request, HotelReview $hotelReview)
    {
        $data = $request->validate([
            'hidden_reason' => 'nullable|string|max:500',
        ], [
            'hidden_reason.max' => 'Lý do ẩn không vượt quá 500 ký tự.',
        ]);

        DB::transaction(function () use ($hotelReview, $data) {
            $hotelReview->update([
                'status' => HotelReview::STATUS_HIDDEN,
                'hidden_by' => Auth::id(),
                'hidden_at' => now('Asia/Ho_Chi_Minh'),
            ...
            BookingLog::create([
                ...
                'description' => 'Admin ẩn đánh giá khách sạn.' . (!empty($data['hidden_reason']) ? ' Lý do: ' . $data['hidden_reason'] : ''),
            ]);
        });
    ...
    public function destroy(HotelReview $hotelReview)
    {
        DB::transaction(function () use ($hotelReview) {
            BookingLog::create([
                ...
                'description' => 'Admin xóa mềm đánh giá khách sạn.',
            ]);
        ...
        return redirect()
            ...
            ->with('success', 'Đã xóa mềm đánh giá.');
    }
}
