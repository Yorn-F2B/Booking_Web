...
use App\Services\BookingRoomPeriodService;
...
class BookingRoomController extends Controller
{
        ...
        try {
            ...
            foreach ($selectedRoomIds as $index => $roomId) {
                ...
                if (!$bookingRoom) {
                    ...
                    // Before check-in this replaces the whole planned stay; after check-in the
                    // public changeRoom action below preserves the completed period.
                    $periods = app(BookingRoomPeriodService::class);
                    $periods->ensureInitial($booking, $bookingRoom);
                    $periods->changeFrom(
                        $booking,
                        $bookingRoom,
                        $newRoom->id,
                        (float) ($newRoom->category?->price ?? $bookingRoom->price_at_booking),
                        $booking->check_in_at?->toDateString() ?? $booking->check_in_date
                    );
    ...
    public function changeRoom(Request $request, Booking $booking)
    {
        ...
        try {
            ...
            $newNightlyPrice = (float) ($newRoom->category?->price ?? $bookingRoom->price_at_booking);
            app(BookingRoomPeriodService::class)->changeFrom(
                $booking,
                $bookingRoom,
                $newRoom->id,
                $newNightlyPrice,
            ...
            BookingRoomChange::create([
                ...
                'new_room_category_id' => $newRoom->room_category_id,
                'old_room_price' => (float) $bookingRoom->price_at_booking,
                'new_room_price' => $newNightlyPrice,
                'night_count' => max(1, Carbon::parse($booking->check_in_date)->diffInDays(Carbon::parse($booking->check_out_date))),
                'price_difference_total' => 0,
                'change_source' => 'front_desk',
