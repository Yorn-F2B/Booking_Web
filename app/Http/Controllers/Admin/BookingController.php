...
class BookingController extends Controller
{
    ...
    public function addGuest(Request $request, Booking $booking)
    {
        ...
        $data = $request->validate([
            ...
            'cccd.required' => 'Vui lòng cung cấp CCCD.',
            'booking_room_id.required' => 'Vui lòng chọn phòng cho khách lưu trú.',
            'booking_room_id.exists' => 'Phòng được chọn không hợp lệ.',
        ]);

        $bookingRoom = \App\Models\BookingRoom::with('room.category')->find($data['booking_room_id']);
        if ($bookingRoom->booking_id !== $booking->id) {
            return back()->withInput()->with('error', 'Phòng không thuộc booking này.');
        }

        $scannedCccd = preg_replace('/\D+/', '', (string) $data['cccd']);
        $primaryCccd = preg_replace('/\D+/', '', (string) ($booking->customer->cccd ?? ''));

        if ($scannedCccd !== '' && hash_equals($scannedCccd, $primaryCccd)) {
            return back()->withInput()->with('error', 'CCCD bị trùng với khách đứng tên booking. Không thể gán lại.');
        }

        $isDuplicate = $booking->guests()->get()->contains(function ($guest) use ($scannedCccd) {
            return hash_equals(preg_replace('/\D+/', '', (string) $guest->cccd), $scannedCccd);
        });

        if ($isDuplicate) {
            return back()->withInput()->with('error', 'CCCD này đã tồn tại trong danh sách khách lưu trú của booking.');
        }

        $currentGuestsCount = $booking->guests()->where('booking_room_id', $bookingRoom->id)->count();
        $adultCapacity = $bookingRoom->room->category->adult_capacity ?? 0;

        if ($currentGuestsCount >= $adultCapacity) {
             return back()->withInput()->with('error', 'Phòng ' . $bookingRoom->room->room_number . ' đã đạt tối đa sức chứa (' . $adultCapacity . ' khách). Yêu cầu Lễ tân thu thêm Phụ thu thêm người tại tab Dịch vụ, hoặc thực hiện Chuyển hạng phòng.');
        }

        if ($request->hasFile('cccd_image')) {
            $data['cccd_image_path'] = $request->file('cccd_image')->store('cccds', 'public');
        }

        $booking->guests()->create($data);

        $message = 'Thêm khách lưu trú: ' . $data['full_name'] . ' vào phòng ' . $bookingRoom->room->room_number;
        if (!empty($data['cccd'])) {
            $message .= ' (CCCD: ' . $data['cccd'] . ')';
        }

        $this->addBookingLog($booking, 'guest_added', $message);
