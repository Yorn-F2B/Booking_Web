...
class BookingPromotionApplicationService
{
    public function apply(
        ...
        foreach ($promotions as $promotion) {
            if (($promotion->coupon_scope ?? 'normal') === 'incident' && !$scopeRoom) {
                throw new \RuntimeException('Mã hỗ trợ sự cố phải được áp cho đúng một phòng (booking_room_id).');
            }
            if (($promotion->coupon_scope ?? 'normal') === 'normal' && $scopeRoom) {
                throw new \RuntimeException('Mã thông thường chỉ áp dụng cho toàn booking.');
            }
        }
    ...
    private function applyNormalPromotions(
        ...
        $roomBaseTotal = 0;
        if ($scopeRoom) {
            $roomBaseTotal = app(BookingCalculatorService::class)->calculateRoom($booking, $scopeRoom)['gross'];
        }
        ...
        if ($scopeRoom) {
            // Incident assistance belongs exclusively to this room and may never exceed it.
            $roomGross = app(BookingCalculatorService::class)->calculateRoom($booking, $scopeRoom)['gross'];
            $addedDiscount = min($addedDiscount, max(0, $roomGross - (float) ($scopeRoom->support_amount ?? 0)));
            $scopeRoom->update([
                'support_amount' => (float) ($scopeRoom->support_amount ?? 0) + $addedDiscount,
                'support_reason' => trim(($scopeRoom->support_reason ? $scopeRoom->support_reason . ' | ' : '') . 'Mã hỗ trợ: ' . $codes->implode(', ')),
            ]);
        }
        // Room-incident aid is represented by booking_rooms.support_amount; never also add
        // it to booking.discount_amount or another room would be discounted a second time.
        $newDiscount = $scopeRoom ? $currentDiscount : $currentDiscount + $addedDiscount;
        ...
        $newTotal = max(0, $newBookingSubtotal - $newDiscount - ($scopeRoom ? $addedDiscount : 0));
